    ...
    def main(self, base_config: dict, target_fitness: float = 1000, 
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            for (i, ind) in enumerate(population):
                ind.fitness.values = self.evaluate_individual(ind, copy.deepcopy(base_config), indiv_number=i, gen_number=0)
        ...
        for gen in tqdm(range(start_gen, self.generations + 1)):
            ...
            for (i, ind) in enumerate(offspring):
                ind.fitness.values = self.evaluate_individual(ind, copy.deepcopy(base_config), indiv_number=i, gen_number=gen)
            # Combine populations
